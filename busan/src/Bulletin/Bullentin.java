...
public class Bullentin extends JFrame {
	private JPanel[] pnlC = new JPanel[100];
	private JLabel[] lblC = new JLabel[100];
	private JLabel[] lblD = new JLabel[100];
	private int total = 0;
	...
	public Bullentin(BusanUser user) {
		super("후기");
		this.user = user;
		
		URL emptyStar = Bullentin.class.getClassLoader().getResource("empty.png");
		URL fullStar = Bullentin.class.getClassLoader().getResource("full.png");
				...
				if (!textField.getText().equals("")) { // 텍스트 필드가 비어있지 않다면
					if (textField.getText().length() >= 70) { // 텍스트 길이가 70자보다 길다면
						...
						if (star5.getIcon().equals(fullimg) && score5 <= 50) {
							...
							JOptionPane.showMessageDialog(Bullentin.this, "별점을 넣어주세요!");
						} else {
							lblC[total].setText(user.getId() + "  :	");
					...
					JOptionPane.showMessageDialog(Bullentin.this, "후기를 입력해주세요!");
				}
